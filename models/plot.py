...
def plot_mull_pop(array1, array2):
    x = np.arange(0, len(array1))  #generates_an_array_of_values_each_number_corresponding_to_an_orientation
    plt.scatter(x, array1, marker='o', color="r", label=" actual data")
    plt.scatter(x, array2, marker='X',  color="black", label="ML prediction")
    plt.ylim(0, 2)
    plt.title('x vs Mulliken population for dataset $\u03C6_{1}$ (SEM)')
    #plt.xticks(x)
    plt.xlabel('X (numerical label of an orientation)')
    plt.ylabel('Mulliken population')
    plt.legend()
    #plt.savefig("plot_sem_for_phi1_mull_pop")                                                                                                                   
    plt.show()
